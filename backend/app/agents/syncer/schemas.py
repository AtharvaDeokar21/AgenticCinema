...
class AudioPlacement(BaseModel):
    ...
    pacing_suggestion: Optional[str] = Field(
        default=None,
        description=(
            "Editor pacing advice generated by comparing the audio clip duration "
            "to the visual window (video_end_time - video_start_time). "
            "Examples: 'Fits well', 'Audio is too long; consider cutting the "
            "dialogue or freezing the video frame', "
            "'Audio is too short; consider slowing the video or adding a pause'."
        ),
    )
...
class SyncMap(BaseModel):
    ...
    editor_timeline_text: str = Field(
        default="",
        description=(
            "A clean, human-readable text block summarising the entire sync map "
            "so an editor can read the placement guide without parsing JSON. "
            "Format each line as: "
            "[MM:SS.mm - MM:SS.mm] -> Attach <filename> (<pacing note if any>). "
            "Example:\n"
            "  [00:01.20 - 00:02.50] -> Attach audio_beat_001.wav\n"
            "  [00:03.00 - 00:04.10] -> Attach audio_beat_002.wav (Stretch video to fit)"
        ),
    )
